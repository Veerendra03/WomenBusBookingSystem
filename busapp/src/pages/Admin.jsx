...
function Admin() {
  ...
  const [logoClicked, setLogoClicked] = useState(false); // State for logo animation
  ...
  const handleSendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/send-otp`, {
        email: formData.email,
        phone: formData.phone,
        role: formData.role,
      });
      showToastNotification(`OTP sent to ${formData.email}. Check your inbox.`); // Updated message
      setOtpSent(true);
    } catch (error) {
  ...
  const handleVerifyOtp = async () => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/verify-otp`, {
        email: formData.email,
        otp: formData.otp,
      });
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/admin-signup`, {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/admin-login`, {
        email: formData.email,
        password: formData.password,
      });
      localStorage.setItem('token', response.data.token);
  ...
  const handleLogoClick = () => {
    setLogoClicked(true);
    setTimeout(() => setLogoClicked(false), 1500); // Reset after animation duration
  };
